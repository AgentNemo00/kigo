...
Modules can be run everywhere you want, `KiGoUI` and `KiGo` (for now) are running on the device connected to the screen. The modules can be run on the same device or at a different device at all. This gives us the benefit of able to expande to heavy loads, i.e. ML/AI/SMLs (your choice). For a distributed architecture some changes would need to me made. `KiGo`will have a REST API which is currently undefined and should integrate `KiGoMa`.
